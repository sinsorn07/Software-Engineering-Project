...
export const register = (req, res) => {
  ...
  const q = "SELECT * FROM users WHERE username = ?"; //we use ? for values instead of writing 'req.body.username' for security

  db.query(q, [req.body.username], (err, data) => {
    if (err) return res.status(500).json(err);
    if (data.length) return res.status(409).json("User already exists!");
    //CREATE A NEW USER
    //Hash the password (ex: 123456bibi => asisdkfj83nvckf200ei), and we will store only hash part
    ...
    db.query(q, [values], (err, data) => {
      if (err) return res.status(500).json(err);
      return res.status(200).json({ message: "User has been created.", success: true });
    });
  });
};
